...
func getMembershipInstances(conn net.Conn) (*MembershipInstances, error) {
	...
	membershipInstancesRawSlice, err := convertToSlice(membershipInstancesRaw)
	if err != nil {
	...
	for _, instanceRaw := range membershipInstancesRawSlice {
		instanceMap, err := convertToMapWithStringKeys(instanceRaw)
		if err != nil {
