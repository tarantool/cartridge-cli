...
func adminFuncList(conn net.Conn) error {
	...
	if len(listResRawMap) == 0 {
		return fmt.Errorf("No avaliable admin functions found")
	}
	...
	for funcNameRaw, funcSpecRaw := range listResRawMap {
		...
		if err != nil {
			return getCliExtError("Failed to ge function %q usage: %s", funcName, err)
		}
	...
	log.Infof("Avaliable admin functions:\n\n%s", funcUsages.Format())
