...
Нам не нужно создавать проект с нуля -- можно воспользоваться уже готовыми
...
На самом деле, в шаблоне гораздо больше объектов -- хотя бы потому, что
...
1. ```deps.sh``` -- скрипт-однострочник для удобной установки зависимостей для
   проекта.
2. ```init.lua``` -- точка входа в наше приложение. В этом примере мы не будем
   подробно разбирать его, однако для разработки более сложных сервисов
   потребуется понимать, что происходит в этом скрипте.
3. ```app/roles``` -- наша основная рабочая директория. В ней удобно описывать
   все роли.
4. ```cartridge-example-scm-1.rockspec``` -- _"рокспека"_, это файл-манифест
...
Роль -- это часть нашего приложения, логически обособленная от других частей.
...
* `init(opts)` -- создание роли и ее инициализация.
* `stop()` -- завершение работы роли;
* `validate_config(conf_new, conf_old)` -- функция валидирования новой
  конфигурации нашей роли;
* `apply_config(conf, opts)` -- применение новой конфигурации.

Сам файл роли -- это просто Lua-модуль, в конце которого
...
* `start.sh` -- для запуска кластера из 5 экземпляров
* `stop.sh` -- для остановки всех инстансов
* `clean.sh` -- для остановки всех инстансов и очистки рабочей директории
...
Написание тестов -- тема для отдельного урока. Сейчас же мы запустим тесты,
